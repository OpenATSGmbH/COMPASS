...
public:
    ...
    // Colors
    // ================================================================
    /// base color; setting it regenerates line_colors_ (use setLineColor to override a single line).
    QColor baseColor() const { return base_color_; }
    void baseColor(const QColor& color);

    /// the four line shades derived from base_color_. Manual overrides persist
    /// until baseColor() is set again, which regenerates all four.
    const std::array<QColor, 4>& lineColors() const { return line_colors_; }
    QColor lineColor(unsigned int line_id) const;
    void setLineColor(unsigned int line_id, const QColor& color);
    /// restore line_colors_[line_id] to the autogenerated shade for base_color_
    void resetLineColor(unsigned int line_id);
...
private:
    ...
    QColor base_color_;
    std::array<QColor, 4> line_colors_;
};
