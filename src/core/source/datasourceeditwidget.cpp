...
void DataSourceEditWidget::createMainTab()
{
    ...
    // ================================================================
    {
        ...
        auto* colors_header = new QLabel("Colors");
        colors_header->setFont(font_bold);
        main_layout->addWidget(colors_header);

        // Base color row
        auto* base_row = new QHBoxLayout();
        base_row->addWidget(new QLabel("Base Color"));

        base_color_button_ = new QPushButton();
        base_color_button_->setIcon(QIcon());
        base_color_button_->setToolTip("Base color for this data source");
        base_color_button_->setMinimumWidth(100);
        connect(base_color_button_, &QPushButton::clicked, this,
                &DataSourceEditWidget::baseColorClickedSlot);
        base_row->addWidget(base_color_button_);

        auto_light_button_ = new QPushButton("Auto Light");
        auto_light_button_->setIcon(QIcon());
        auto_light_button_->setToolTip("Generate a light-band base color and re-derive line colors");
        connect(auto_light_button_, &QPushButton::clicked, this,
                &DataSourceEditWidget::autoLightClickedSlot);
        base_row->addWidget(auto_light_button_);

        auto_dark_button_ = new QPushButton("Auto Dark");
        auto_dark_button_->setIcon(QIcon());
        auto_dark_button_->setToolTip("Generate a dark-band base color and re-derive line colors");
        connect(auto_dark_button_, &QPushButton::clicked, this,
                &DataSourceEditWidget::autoDarkClickedSlot);
        base_row->addWidget(auto_dark_button_);

        base_reset_button_ = new QPushButton("Reset");
        base_reset_button_->setIcon(QIcon());
        base_reset_button_->setToolTip("Regenerate base color using the context's color preference");
        connect(base_reset_button_, &QPushButton::clicked, this,
                &DataSourceEditWidget::resetBaseColorClickedSlot);
        base_row->addWidget(base_reset_button_);

        base_row->addStretch();
        main_layout->addLayout(base_row);

        // Line colors row
        auto* line_row = new QHBoxLayout();
        line_row->addWidget(new QLabel("Line Colors"));

        for (unsigned int i = 0; i < line_color_buttons_.size(); ++i)
        {
            auto* btn = new QPushButton(QString("L%1").arg(i + 1));
            btn->setIcon(QIcon());
            btn->setToolTip(QString("Color for line L%1 — click to edit").arg(i + 1));
            btn->setProperty("line_id", i);
            btn->setMinimumWidth(60);
            connect(btn, &QPushButton::clicked, this,
                    &DataSourceEditWidget::lineColorClickedSlot);
            line_color_buttons_[i] = btn;
            line_row->addWidget(btn);

            auto* reset = new QPushButton("Reset");
            reset->setIcon(QIcon());
            reset->setToolTip(QString("Restore autogenerated shade for L%1").arg(i + 1));
            reset->setProperty("line_id", i);
            connect(reset, &QPushButton::clicked, this,
                    &DataSourceEditWidget::lineColorResetClickedSlot);
            line_reset_buttons_[i] = reset;
            line_row->addWidget(reset);
        }
...
void DataSourceEditWidget::applyColorSwatch(QPushButton* button, const QColor& color)
{
    if (!button)
        return;
...
void DataSourceEditWidget::updateColors(context::DataSource* ds)
{
    bool have_ds = (ds != nullptr);

    base_color_button_->setEnabled(have_ds && !read_only_);
    auto_light_button_->setEnabled(have_ds && !read_only_);
    auto_dark_button_->setEnabled(have_ds && !read_only_);
    base_reset_button_->setEnabled(have_ds && !read_only_);

    for (auto* b : line_color_buttons_)
        if (b) b->setEnabled(have_ds && !read_only_);
    for (auto* b : line_reset_buttons_)
        if (b) b->setEnabled(have_ds && !read_only_);

    if (!have_ds)
    {
        applyColorSwatch(base_color_button_, QColor());
        base_color_button_->setText("—");
        for (auto* b : line_color_buttons_)
            applyColorSwatch(b, QColor());
        return;
    }

    applyColorSwatch(base_color_button_, ds->baseColor());
    if (!ds->baseColor().isValid())
        base_color_button_->setText("—");

    for (unsigned int i = 0; i < line_color_buttons_.size(); ++i)
    {
        applyColorSwatch(line_color_buttons_[i], ds->lineColor(i));
        if (!ds->lineColor(i).isValid())
            line_color_buttons_[i]->setText(QString("L%1").arg(i + 1));
    }
}

void DataSourceEditWidget::baseColorClickedSlot()
{
    if (read_only_ || !current_ds_)
        return;
...
void DataSourceEditWidget::autoLightClickedSlot()
{
    if (read_only_ || !current_ds_)
        return;
    ...
    if (ctx_man_ && ctx_man_->hasActiveContext())
    {
        for (const auto& other : ctx_man_->activeContext().dataSources())
        {
            if (other.id() == ds.id()) continue;
            if (other.dsType() != ds.dsType()) continue;
            if (other.baseColor().isValid())
                existing.push_back(other.baseColor());
        }
    }

    ds.baseColor(context::ColorProvider::generateBaseColor(
        existing, context::ColorProvider::Band::Light, ds.dsType()));

    loginf << "ds " << ds.id() << " auto light base color " << ds.baseColor().name().toStdString();

    updateColors(&ds);
    update_ds_func_(ds.id());
}

void DataSourceEditWidget::autoDarkClickedSlot()
{
    if (read_only_ || !current_ds_)
        return;
    ...
    ds.baseColor(context::ColorProvider::generateBaseColor(
        existing, context::ColorProvider::Band::Dark, ds.dsType()));

    loginf << "ds " << ds.id() << " auto dark base color " << ds.baseColor().name().toStdString();

    updateColors(&ds);
    update_ds_func_(ds.id());
}

void DataSourceEditWidget::resetBaseColorClickedSlot()
{
    if (read_only_ || !current_ds_)
        return;
    ...
    // use the context preference via autoAssignColors — first invalidate so it regenerates
    ds.baseColor(QColor());
    if (ctx_man_)
        ctx_man_->autoAssignColors(ds);
    else
        ds.baseColor(context::ColorProvider::generateBaseColor({}, context::ColorProvider::Band::Light));

    loginf << "ds " << ds.id() << " reset base color " << ds.baseColor().name().toStdString();

    updateColors(&ds);
    update_ds_func_(ds.id());
}

void DataSourceEditWidget::lineColorClickedSlot()
{
    if (read_only_ || !current_ds_)
        return;
    ...
    QColor color = QColorDialog::getColor(initial, this, QString("Line L%1 Color").arg(line_id + 1));
    if (!color.isValid())
        return;

    loginf << "ds " << ds.id() << " line " << (line_id + 1) << " color " << color.name().toStdString();
...
void DataSourceEditWidget::lineColorResetClickedSlot()
{
    if (read_only_ || !current_ds_)
        return;
    ...
    auto& ds = currentDataSource();
    ds.resetLineColor(line_id);

    loginf << "ds " << ds.id() << " line " << (line_id + 1) << " reset";
