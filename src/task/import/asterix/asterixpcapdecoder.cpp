                                       ...
                                       std::string& warning) const
{
    ...
    //store categories found in the section but skipped during decoding
    section.skipped_categories = ASTERIXSkippedCategoryInfo::fromAnalysisInfo(*analysis_info);

    if (!num_records) // no data
    {
        if (!section.skipped_categories.empty())
            error = "Only skipped categories: "
                    + ASTERIXSkippedCategoryInfo::asString(section.skipped_categories);
        else
            error = "Decoding failed";
        return false;
    }
    ...
    for (const auto& cat_it : section.skipped_categories)
