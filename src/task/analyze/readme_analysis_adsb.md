...
The cadence is **motion-adaptive**, because 1090ES surface squitter is: an aircraft transmits at the high rate while taxiing (~1 s) and drops to a low rate when stopped (~5 s, per DO-260B). Two nominal intervals are therefore configured - `update_interval_s_` (moving, default 1.0 s) and `update_interval_standing_s_` (standing, default 5.0 s) - and each expected slot / gap is classified standing or moving by the target's ground speed at that time: **ADS-B-reported `ground_speed` where available, RefTraj ground speed as fallback; unknown speed counts as moving** (matching `ReconstructorInfo::isMoving()`). A slot is standing when that speed is below `standing_speed_max_mps_` (default 0.5 m/s; COMPASS's own `GroundSpeedMin` is 0.257 m/s). This keeps a parked aircraft from being counted as missing simply because it squitters less often - without it, the overall PD on a busy apron collapses (e.g. on LOWW surface data, 0.44 with a flat 1 s UI vs 0.73 with the adaptive UI, median per-cell PD 0.96 vs 0.99). The PD output stays a single combined number; only the expected-update denominator is movement-aware.
...
There is no CAT019 cadence variant for ADS-B. The slot-walk is, however, **speed-adaptive**: expected slots are stepped by the local update interval (moving or standing) rather than a single fixed UI, and the miss slots inside a report gap are stepped by the same per-slot rule (a slot counts as missed when it lies more than the miss tolerance before the next report). Standing vs moving is decided from the ADS-B-reported ground speed (RefTraj speed as fallback) per the [Cadence source](#feature-2-sensor-coverage-analysis--pd-adsb-free-per-transponder) rule above. An earlier version froze the miss cadence at the gap-start classification, which over-counted long park-after-moving gaps at the moving cadence and produced negative PDs (more missed than expected slots); stepping the miss slots adaptively keeps the miss count consistent with the expected-slot count by construction.

### Reported accuracy derivation (Feature 3)

The reported `tr_std_dev` is obtained from the target report's `posAccuracy()` accessor, which performs the QI-to-stddev lookup the reconstructor uses (NACp primary, NIC fallback, NUCp for v0; `/2.45`; SIL penalty), so the offline analysis and the online estimator agree by construction. The branch order and conversion are described in the [readme_adsb_accuracy.md](../../../readme_adsb_accuracy.md). The `qi_key` used for the per-transponder sub-buckets and the dubious detail is derived from the raw `mopsVersion` / `nacp` / `nucpNic` values, mirroring `ADSBAccuracyEstimator::qiKey`.
