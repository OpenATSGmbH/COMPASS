...
2 different reconstructors exist, but the advanced reference trajectory calculation feature (Probabilistic + IMM Reconstructor) is only available under a commercial professional license. In the free license, a basic reconstructor (Scoring + UMKalman) is available. Please refer to \nameref{sec:ui_configure_licenses} for licensing information. \\
...
\item Computes the marginal probabilities of all feasible measurement-to-target associations, considering under clutter and missed detections
...
However, this calculation requires defined minimum and maximum accuracy parameters, otherwise the combination with Accuracy Scaling may lead spiraling into associating nothing or everything, depending on the clutter level.\\
