...
Two different reconstructors exist, but the advanced reference trajectory calculation feature (Probabilistic + IMM Reconstructor) is only available under a commercial professional license. In the free license, a basic reconstructor (Scoring + UMKalman) is available. Please refer to \nameref{sec:ui_configure_licenses} for licensing information. \\
...
\begin{itemize}  
   ...
   \item Uses Joint Probabilistic Data Association (JPDA), Radar bias estimation \& correction, ADS-B geometric altitude for slant range correction, accuracy re-estimation and scaling, ADS-B verification, ...
...
In the 'Probabilistic + IMM' reconstructor, each data slice is processed multiple times, to re-estimate correctable biases as well as to rescale target report accuracies. \\
...
If the 'Do Identity Change Cut' parameter is set, system tracks are additionally checked for permanent Mode 3/A code changes, as caused by a track swap (seduction) onto a different target. 
A change is only accepted as such if the new code is sustained, the old code ceases, neither code is a conspicuity code, the Mode S address and identification do not indicate the same aircraft on both sides, and another tracker data source observed the old identity without performing the same change (which would indicate a regular code change of the same aircraft). 
At an accepted identity change, the track number to UTN association is cut: the target reports of the wrongly continued track part are separated into an own UTN, avoiding the merging of 2 different real-world targets into one UTN. \\
...
A target type is defined based on the following information (if available):
...
This is of course not well-suited for strongly varied sensor accuracies and separations (e.g. when mixing ground and air surveillance data). \\



\subsection{Probabilistic + IMM Features}
...
\item Using parameter-based models as well as 2D accuracy maps
\end{itemize}
...
\begin{itemize}
\item Method based on published paper: \href{https://doi.org/10.1109/CDC.1980.271915}{“Multi-Target Tracking Using Joint Probabilistic Data Association” by Fortmann, Bar-Shalom \& Scheffe, 1980}
\item Computes the marginal probabilities of all feasible measurement-to-target associations, considering clutter and missed detections
\item Updates each target state by weighting measurements by those probabilities and saves optimal associations
\end{itemize}
...
\begin{itemize}
\item Based on estimated target position estimate and (re-scaled) target report accuracy
\end{itemize}
...
\subsubsection{Automatic Radar Bias Estimation}
\label{sec:reconst_radar_bias_estimation}

During reconstruction with the Probabilistic + IMM reconstructor, range and azimuth biases are estimated per radar Data Source from the available reference trajectory. The estimated biases are written back into the active Data Context for that Data Source, so they are visible in the Data Context dialog (see \nameref{sec:configure_data_context}) and reused as inputs on subsequent reconstructor runs. The behavior is closed-loop: no manual bias entry is required in the normal workflow. \\

The behavior is enabled by the 'Correct Radar Biases' checkbox in the reconstructor configuration UI; when disabled, no bias is estimated, applied or persisted. See \nameref{sec:reconst_config}.

\subsubsection{Simple Reconstructor Improvements for Primary-Only Data}

The basic Scoring + UMKalman reconstructor has been improved in handling primary-only (PSR) and Mode A/C-only radar association and reconstruction. This affects internal scoring and association decisions only; no additional user-facing controls have been added. \\

\subsubsection{Probabilistic Position Matching}

Generally, static position thresholds are avoided in favor of probabilistic ones. The system employs probabilistic distance scoring utilizing Mahalanobis distances, which are based on position accuracy estimates. Thresholds are then applied to these Mahalanobis distances (e.g. 3x Standard Deviation).\\

The \href{https://en.wikipedia.org/wiki/Mahalanobis_distance}{Mahalanobis distance} measures the distance between a point $P$ and a probability distribution $D$. Effectively, it indicates how many standard deviations $\sigma$ the point $P$ is away from the mean of $D$. This metric divides the distance between the target report and the target by the sum of accuracies (standard deviations). \\

However, this calculation requires defined minimum and maximum accuracy parameters, otherwise the combination with Accuracy Scaling may lead to spiraling into associating nothing or everything, depending on the clutter level.\\

Using this distance, different match qualities can be defined: for example, an acceptable match at 3$\sigma$, a dubious match at 4$\sigma$, and an outlier at 5$\sigma$. Different thresholds can be configured for primary-only versus secondary associations, operating effectively both on ground and in-air.\\

Crucially, this method relies on accurate estimates of target report and target state standard deviations, which are obtained through accuracy rescaling and multiple time-slice processing.

\subsubsection{JPDA for PSR/SMR plots}

Joint Probabilistic Data Association (JPDA) is applied for PSR and SMR plots to facilitate position-based association (\href{https://doi.org/10.1109/CDC.1980.271915}{“Multi-Target Tracking Using Joint Probabilistic Data Association” by Fortmann, Bar-Shalom \& Scheffe, 1980}). JPDA is an adequate solution because it provides improved position estimates in high clutter situations. By considering all feasible associations and determining the most likely hypothesis for primary plot association, it mitigates the impact of false detections and clutter. \\

This probabilistic approach, when used in combination with Probabilistic Position Matching, ensures that the reconstructed reference trajectory remains stable and accurate even in challenging detection environments. \\

Using Full Primary-Only Tracking, primary-only plots are not only associated to matching targets, but can generate new primary-only targets. While reconstructing a dataset with only primary-only sensors is not recommended, this feature will allow formation of primary-only UTNs based on primary-only detections only (without e.g. a related system track or secondary detection).

\subsubsection{Radar Plot Position Improvements}
...
\subsubsection{Rescaling Accuracies}
...
\begin{figure}[H]
    \centering
      \includegraphics[width=12cm,frame]{figures/process_in_progress.png}
    \caption{Reconstructing References Task - In Progress}
\end{figure}

The processing can be canceled using the 'Cancel' button, which might take a few seconds. \\

After all data slices are processed and the calculated references associations are saved, the task is done.

\begin{figure}[H]
    \centering
      \includegraphics[width=12cm,frame]{figures/process_done.png}
...
\begin{figure}[H]
    \centering
    \hspace*{-2.5cm}
    \includegraphics[width=19cm,frame]{figures/created_targets.png}
