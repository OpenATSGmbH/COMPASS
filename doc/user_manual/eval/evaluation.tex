...
\textcolor{red}{\textbf{[TODO: update screenshot figures/eval\_standard.png: new elements 'Ignore Primary Only Targets' and 'Ignore Non-ADSB Targets']}} \\

The 2 target settings define which targets are relevant for the standard. A standard for secondary surveillance, e.g. \nameref{sec:eval_req_detection} of an MLAT system, cannot be assessed with targets that carry no transponder, and an ADS-B standard cannot be assessed with targets that never transmit ADS-B. After the evaluation data is loaded, the current standard checks each target and marks the not relevant ones as ignored. \\
...
\item Ignored targets are listed in the results as unusable results, with the reason 'Primary-only' or 'No ADS-B data'. They do not contribute to the sector sums.
\end{itemize}
\ \\
