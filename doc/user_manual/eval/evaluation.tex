...
To give a few examples of what this could mean:
\begin{itemize}  
...
\subsubsection{Minimum Reference Accuracy}
\label{sec:eval_min_ref_acc}

For the position requirements, the accuracy of the reference itself is checked before a test target report is evaluated. The position standard deviation of the reference at the interpolation time is derived from the two reference updates surrounding the test target report, using the worse of the two values (each value being the larger of the X and Y component standard deviations). \\

If this reference standard deviation is larger than the 'Minimum Reference Accuracy [m]' value defined in the respective requirement, the reference is considered too inaccurate to judge the test data at that time. The test target report is then skipped, counting neither as passed nor as failed, and is listed in the requirement results as '\#RefPosIn' (number and percentage of updates with inaccurate reference position). \\

The value is therefore an upper limit on the acceptable reference position standard deviation. The default value of 10 meters means that only test target reports for which the reference is accurate to a standard deviation of 10 meters or better are checked. Lowering the value makes the evaluation stricter regarding reference quality, but excludes more test data, while raising it includes more test data at the cost of accepting a less accurate reference. \\

The value can be changed per requirement in the 'Standard' tab, in the configuration of the respective position requirement.
...
\item Reference Data:
\begin{itemize}
\item DBContent: Any DBContent existing in the database, except CAT001 and CAT048 (see note below)
...
\textbf{Note}: CAT001 and CAT048 cannot be selected as reference data. Reference accuracy filtering (see \nameref{sec:eval_min_ref_acc}) requires a maximum X/Y standard deviation, which these categories do not provide. They can still be used as test data. \\
...
At the bottom, the following general settings of the current standard can be edited:
\begin{itemize}
\item Reference Maximum Time Difference [s]: Maximum time difference between reference updates. This value is used to find time-adjacent reference target reports for test target reports. Please adjust this value to e.g. the reference update period plus 1 second.
\item Ignore Primary Only Targets: Targets without any secondary attributes (no Mode S address, no aircraft identification, no Mode 3/A code, no Mode C code) are ignored
\item Ignore Non-ADSB Targets: Targets never detected in ADS-B data (CAT021) are ignored
\item Ignore Mode A/C Only Targets: Targets with a Mode 3/A code or a Mode C code, but without any Mode S attribute, are ignored
\end{itemize}
\ \\

\textcolor{red}{\textbf{[TODO: update screenshot figures/eval\_standard.png: new elements 'Ignore Primary Only Targets', 'Ignore Non-ADSB Targets' and 'Ignore Mode A/C Only Targets']}} \\

The 3 target settings define which targets are relevant for the standard. A standard for secondary surveillance, e.g. \nameref{sec:eval_req_detection} of an MLAT system, cannot be assessed with targets that carry no transponder, an ADS-B standard cannot be assessed with targets that never transmit ADS-B, and a standard stating its requirements for Mode S targets only cannot be assessed with Mode A/C-only targets. After the evaluation data is loaded, the current standard checks each target and marks the not relevant ones as ignored. \\

\begin{itemize}
\item The ignored state comes from the current standard and is not stored in the target. The target usage set in the Targets tab, or by the 'Filter UTNs' function, is not changed. Selecting a different standard therefore changes which targets are ignored, without any manual correction.
\item Ignored targets are listed in the results as unusable results, with the reason 'Primary-only', 'No ADS-B data' or 'Mode A/C-only'. They do not contribute to the sector sums.
\end{itemize}
\ \\

\includegraphics[width=0.5cm]{../../data/icons/hint.png} \textbf{Please note that} 'Ignore Non-ADSB Targets' uses the ADS-B data in the database as the only evidence. If the database contains a single ADS-B data source, and this source is the one under test, a target that the source never received is ignored as well, although it can be an ADS-B equipped target that was not detected. In this case, check the ignored targets before the assessment. \\
...
\item EUROCAE ED-117/A (\href{https://eshop.eurocae.net/eurocae-documents-and-reports/ed-117a/}{Link}), see \nameref{sec:appendix_eval_std_ed117} and \nameref{sec:appendix_eval_std_ed117a}
\item EUROCAE ED-129C (\href{https://eshop.eurocae.net/eurocae-documents-and-reports/ed-129c/}{Link})
...
For the EUROCAE ED-117 and ED-117A standards, the mapping of each requirement of the source document onto a COMPASS requirement is documented in \nameref{sec:appendix_eval_standards}. Both definitions were checked against the source documents and assessed with surface recordings. The two limitations stated in earlier versions of this document have since been addressed:
\begin{itemize}
\item The position accuracy on stands is calculated over a 5 second averaging window, using the 'Use Averaging' option of the \nameref{sec:eval_req_pos_distance} requirement.
\item A lower update rate for non-moving targets is supported by the 'Use Stationary Update Interval' option of the \nameref{sec:eval_req_detection} requirement. It is not set in the supplied ED-117/A definitions, since neither document states what constitutes non-movement. It can be set per requirement where the assessed system is known to behave this way.
\end{itemize}
\ \\

For the EUROCAE ED-129C standard (ADS-B Ground Station performance), the following applies:
\begin{itemize}
\item The Probability of Update and Ratio of Long Gaps requirements use the \nameref{sec:eval_req_detection} requirement with the time-based calculation (ED-129C Appendix C 'Interarrivaltime' method).
\item The latency requirements (\nameref{sec:eval_req_adsb_latency}) require CAT021 test data with the data items I021/073 (Time of Message Reception for Position) and I021/077 (Time of Report Transmission).
\item The ACID and Mode A change delay recommendation (REC 495) is checked with the \nameref{sec:eval_req_id_change_delay} requirement.
\item The requirements regarding time stamp accuracy relative to UTC (REQ 20, REQ 21, REQ 308) cannot be evaluated from recorded data and are not part of the standard definition.
\end{itemize}
\ \\

The other standards have not yet been tested to the fullest degree, which is a work in progress.
...
\begin{center}
 \begin{table}[H]
  \begin{tabularx}{\textwidth}{ | l | X |  }
    ...
    \nameref{sec:eval_req_adsb_latency} & Calculates the probability of the ADS-B Ground Station latency being within a threshold (based on test data only) \\ \hline
    ...
    \nameref{sec:eval_req_id_change_delay} & Calculates probability of acceptable identification change delay \\ \hline
...
If a requirement is clicked, its configuration widget is shown on the right hand side. \\
...
\item Target Selection: Target class the requirement is evaluated for
...
The 'Target Selection' defines which targets the requirement is evaluated for:
\begin{itemize}
\item All Targets: Every target, the default
\item Cooperative Only: Targets with a Mode 3/A code, a Mode C code or a Mode S attribute
\item Non-cooperative Only: Targets with none of those, detected by primary means only
\end{itemize}
\ \\

Standards state some requirements for cooperative targets only, and others for every target. A target of another class than the selected one is reported as an unusable result with the reason, and does not contribute to the sector sum. The same target may therefore be assessed by one requirement and skipped by another. This selection is applied in addition to the target filters of the 'Standard' tab, see \nameref{sec:eval_standard}. \\
\ \\
