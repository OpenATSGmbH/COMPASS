...
The next level of detail is the sector sum details, located in 'Sectors$\rightarrow$Sector Layer Name$\rightarrow$Requirement Group Name$\rightarrow$Requirement Name'. \\

The lowest level is the per-target details, located in 'Targets$\rightarrow$UTN' and the respective per-target results located in 
...
The columns of this table are the columns of the report's Report Table for the respective requirement (see \nameref{sec:report_tables}), 
and a 'Comment' column derived from the row values. The table is read from the database when the section is opened. \\

\textcolor{red}{\textbf{[TODO: update screenshot figures/eval\_results\_target\_example.png: the per-target-report table now shows the Report Table columns and the 'Comment' column]}} \\
