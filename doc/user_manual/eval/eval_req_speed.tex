...
In another variation, if the 'Use Percent Threshold if Higher' checkbox is set, the check is changed for faster speeds. If the 'Threshold Percent' percentage of the reference speed is larger than the defined threshold value, the threshold value is replaced by that percentage of the reference speed. This means that for higher speeds, an accuracy with the given percentage is required. \\
...
The offset of the acceleration (test vs. linear interpolated reference values) is used to calculate the offset. If the value of this offset fails the defined comparison threshold, the target report is counted for the calculated probability PCAcc (probability of correct acceleration). The PCAcc must in turn pass the check for the requirement to pass. 
