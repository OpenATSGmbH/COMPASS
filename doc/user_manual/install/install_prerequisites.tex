...
The binary will currently only be provided for (somewhat current) Linux 64-bit operating systems. The AppImage is built on Debian 10 and links against glibc 2.28, so any distribution shipping glibc 2.28 or newer should work. \\
For the following distributions, the AppImage was reported to be working correctly or is expected to be compatible:

\begin{itemize}
\item Debian 10 or later
\item Ubuntu/Mint 20.04 or later
\item RHEL, CentOS Stream, AlmaLinux or Rocky Linux 8 or later
\item Fedora 29 or later
\end{itemize}
\ \\

For the following distributions, the AppImage was reported to have issues (glibc versions too old):

\begin{itemize}
\item CentOS 6
\item RHEL / CentOS 7
\item Ubuntu 18.04 / Linux Mint 19.x
\end{itemize}
\ \\

If you have tried other operating systems than the ones listed here, it would be appreciated if you could provide feedback about your experience to \href{mailto:compass@openats.at}{compass@openats.at}.\\

Other operating systems (e.g. Windows or Mac) are currently not supported.

\subsection{Required Hardware}

The required hardware strongly depends on the use cases, for simple visual inspection of smallish datasets (small number of million target reports) the following minimum requirements exist:
...
Depending on the loaded data size a better CPU, more RAM or a better graphics card are definitely of advantage. \\

\includegraphics[width=0.5cm]{../../data/icons/hint.png} Please \textbf{note} that other graphics cards (Intel, Matrox) will in all probability also work, but are not officially supported. \\\\

An optimal setup could be similar to:

\begin{itemize}  
\item Intel i5 or better (at least 2 physical cores)
\item Decent ATI or NVidia graphics card, e.g.
\begin{itemize}  
...
\item OpenGL core profile version string: This is the OpenGL version, 3.3 (Core Profile) or later is required.
\item OpenGL core profile shading language version string: This is the GLSL version, 3.30 or later is required.
\end{itemize}
