...
A \textbf{Data Context} is a named surveillance context grouping the entities that describe the surveillance environment a database was recorded in: data sources, sectors, FFTs, ASTERIX decoding settings and coloring. It is distinct from the program-wide configuration described above, and 
therefore does not have to be re-created after a version upgrade. \\\\

At most one Data Context is active at a time. When no context is available (e.g. after deleting the last one), the application runs in a 'No Context' state, in which parts of the menu are reduced. \\

\begin{itemize}
\item Groups the following content
\begin{itemize}
\item Data sources (sensor definitions, ranges, accuracies, network settings)
\item Sectors (per-sector airspace polygons grouped by sector layer)
\item Fixed Field Transponders (FFTs)
\item ASTERIX decoding settings per CAT
\item Colors (DSType palette, DBContent palette, per-data-source base and line colors)
\end{itemize}
\item At most one Data Context is active at a time
\begin{itemize}
\item 'No Context' state possible with reduced menu availability
\end{itemize}
\item Persisted under \texttt{\textasciitilde/.compass/data\_contexts/<name>/}
\begin{itemize}
\item One JSON file per content type
\item Active context recorded in \texttt{active\_context.json}
\end{itemize}
\item Switching the active Data Context via the 'Context' menu in the menu bar
\item Managing the active Data Context (edit, copy, rename, delete, import/export ZIP) via the dedicated Data Context dialog, see \nameref{sec:configure_data_context}
\end{itemize}
...
The 'Live: Running' mode can be paused into the 'Live: Paused' mode, which allows offline-like inspection, after which the mode can be resumed into the 'Live: Running' mode without loss of ASTERIX data. \\
...
Selected advanced features require a commercial professional license. The split is as follows: \\

Available in the free license:
\begin{itemize}
\item Basic reconstructor (Scoring + UMKalman) for Reference Trajectory calculation
\item Analyze Data Source MLAT inspectors:
\begin{itemize}
\item 'Data Item Analysis': per-source, per-CAT cumulative table of which ASTERIX data items are present
\item 'Sensor Coverage': per-cell Probability of Detection on a 3D grid using the Reference Trajectory as truth
\end{itemize}
\end{itemize}
\ \\
...
\begin{itemize}
\item Advanced reconstructor (Probabilistic + IMM) for Reference Trajectory calculation
\item Analyze Data Source MLAT inspectors:
\begin{itemize}
\item 'Position Accuracy': maps reported position accuracy, offset against the Reference Trajectory, and consistency between the two
\item 'RU Coverage': per-RU coverage map and Dominant RU map (requires CAT020 with contributing-receiver info)
\item 'RU Effect': per-RU contribution to the MLAT solution via marginal effect table, redundancy curve, and per-RU drill-down
\end{itemize}
\item DOCX evaluation report export
\end{itemize}
\ \\

For entering and verifying the professional license key, see \nameref{sec:ui_configure_licenses}. \\

\subsection*{Data Source Analysis}

Separately from evaluation, COMPASS provides a dedicated 'Analyze Data Source' task that inspects a single data source against the calculated Reference Trajectory from multiple angles. \\\\

The task is independent of the requirement-based evaluation: it does not assess compliance to a standard, but produces per-DSType diagnostics intended to characterize the sensor's behavior in the recorded environment. \\

\begin{itemize}
\item Uses the calculated Reference Trajectory as ground truth
\item Per-DSType inspectors examine a chosen data source from multiple angles
\item Two inspector tiers
\begin{itemize}
\item Free: always available
\item \textbf{[Pro]}: requires a Professional license
\end{itemize}
\item For the full chapter, see \nameref{chap:analyze}
\end{itemize}
...
\subsection*{Report Tables \& Report Variables}

Evaluation reports and Analysis reports store the values computed per target report in the database, as so-called \textbf{Report Tables}. 
Their columns are offered as \textbf{Report Variables} in the variable selection, next to the DBContent variables and Meta Variables (see \nameref{sec:report_tables}). \\

\begin{itemize}
\item One row per test target report or detection gap, one column per computed value
\item Written by every Evaluation and Analyze Data Source run
\item Report Variables
\begin{itemize}
\item Usable in all Views, labels and filters
\item Displayed as 'Report Name: Variable Name'
\end{itemize}
\end{itemize}
...
A View Point bundles filter settings, view configurations, and displayable annotations into a single, restorable view onto either internal database data or externally provided data. \\

Optional components:

\begin{itemize}
    \item \textbf{Data Sources}: which sources to enable
    \item \textbf{Filters}: filter statements for the data load
    \item \textbf{View Configurations}: regions of interest, time frames, per-view settings
    \item \textbf{Annotations}: scatter data, histograms, grid data, geometric data, etc.
\end{itemize}
\ \\

Each component is optional, so a View Point may carry only filters, or only annotations. View Points are stored as JSON; the tag set is documented in \nameref{sec:appendix_view_points}. Import, export, and stepping are handled in the Flight Deck \nameref{sec:view_points} tab, and View Points are also used internally to distribute evaluation results to the Views. \\
