...
\begin{figure}[H]
    \centering
    \includegraphics[width=14cm]{figures/portal_architecture.png}
  \caption{Portal architecture}
\end{figure}

COMPASS Portal does not read data through a running COMPASS process. Finalized per-day databases are opened directly by the Portal middleware. For tasks that require a COMPASS process (configuration, import, reconstruction, evaluation, analyze, export), job execution is delegated to a host-side runner that owns one COMPASS instance and forwards commands to it through the Runtime Command Interface (see \nameref{sec:rtcommand_interface}). \\
