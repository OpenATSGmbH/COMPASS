...
On the left side, the Table View shows a single unified table. Which DBContents, Data Sources and lines are displayed is controlled by the shared Layers Tree in the Configuration Panel (see \nameref{sec:layers_tree}). \\

On the right side resides the Configuration Panel, which contains the Layers Tree, view-specific options and the variable selection.
...
Please \textbf{note} that the earlier per-DBContent tabs and the separate 'All' tab have been replaced by this single unified table. The previous 'All' behavior is now the only mode of operation.
...
Once updated, the table is filled with text representing the values of the chosen DBContent variables. If a value is undefined its cell remains empty. Target reports from multiple DBContents appear interleaved in the same sorted table, allowing cross-comparison workflows that were not possible with the previous per-tab layout. \\

\begin{figure}[H]
  \centering
    \fbox{\parbox{0.8\textwidth}{\vspace{1.5cm}\centering \textcolor{red}{\textbf{[FIGURE TODO: unified Table View with the shared Layers Tree visible on the right and several DBContents enabled, showing interleaved rows]}}\vspace{1.5cm}}}
  \caption{Table View: unified table with multiple layers}
\end{figure}

\begin{figure}[H]
  \centering
    \fbox{\parbox{0.8\textwidth}{\vspace{1.5cm}\centering \textcolor{red}{\textbf{[FIGURE TODO: same Table View with only one layer active in the Layers Tree]}}\vspace{1.5cm}}}
  \caption{Table View: single layer active}
\end{figure}

Please \textbf{note} that since a specific variable might only exist in certain DBContents, cells for rows where the variable does not exist remain empty.

\section{Sorting}

Each column header can be left-clicked to sort the table by that column. Clicking the same header again toggles the sort direction; clicking a different header switches the sort to that column. Only a single column is sorted at a time. \\

\begin{figure}[H]
  \centering
    \fbox{\parbox{0.8\textwidth}{\vspace{1.5cm}\centering \textcolor{red}{\textbf{[FIGURE TODO: close-up of a column header showing the Qt sort indicator arrow]}}\vspace{1.5cm}}}
  \caption{Table View: column sort indicator}
\end{figure}

By default, the table is sorted by Timestamp ascending. \\

\section{Color Mode and Annotations}

The Table View follows the global Color Mode setting (see \nameref{sec:color_mode}). Annotations are not shown in the Table View - the 'Annotations' group of the shared Layers Tree is empty in this view. \\
...
After adding a variable the dataset has to be reloaded to include the additional data, therefore the 'Reload' button becomes active.
...
When this checkbox is checked, the so called presentation mode is used. In the database, the variables might have different units or data representations which are not easy to read.
...
In particular, with Use Presentation enabled, the CAT020 'Contributing Receivers' column shows RU names (multi-line if needed) instead of numerical indexes, provided RU names are defined on the parent MLAT data source (see \nameref{sec:ui_configure_ds_mlat_rus}). When no RU names are defined, indexes remain. \\

\begin{figure}[H]
  \centering
    \fbox{\parbox{0.8\textwidth}{\vspace{1.5cm}\centering \textcolor{red}{\textbf{[FIGURE TODO: Table View with Use Presentation enabled, showing the Contributing Receivers column with RU names instead of indexes]}}\vspace{1.5cm}}}
  \caption{Table View: MLAT Contributing Receivers with names}
\end{figure}
...
The data from the current table can be exported to a comma-separated value (CSV) text, either as file or copied to the clipboard. The export covers a single combined CSV containing all visible buffers, and reflects the current Layers Tree visibility and the active sort order. \\
