...
The table listing the targets contains multiple columns, which are divided into groups.
...
\begin{itemize}  
  ...
  \item MOPS: ADS-B MOPS version and target report count
\end{itemize}
\end{itemize}
\ \\
...
Commonly, all targets are used in an evaluation. However, there are several methods how to de-select certain parts of the evaluated data:
\begin{itemize}
  \item Global: 'Edit Global Excluded Time Windows'
\begin{itemize}
  \item Specify begin/end time: Nothing outside this time is evaluated
  \item Specify exclude time window(s): Nothing inside these time windows is evaluated
 \end{itemize}   
  \item For certain target(s)
\begin{itemize}
  \item Exclude targets fully: Nothing is evaluated (for the given targets)
  \item Specify exclude time window(s): Nothing inside these time windows is evaluated (for the given targets)
  \item Specify exclude requirements: These requirements are not evaluated (for the given targets)
 \end{itemize}   
 \end{itemize} 
 \  \\

The 'Evaluation' submenu provides global operations for setting Evaluation target usage and constraints for all listed targets.
...
\label{sec:ui_eval_global_excl_tw}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Specify begin timestamp, end timestamp and an optional comment
  \end{itemize} 
\end{itemize} 
\ \\
...
\label{sec:ui_eval_target_excl_tw}
...
\label{sec:ui_eval_target_excl_req}
