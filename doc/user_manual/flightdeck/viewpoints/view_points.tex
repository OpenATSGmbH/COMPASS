...
By default, the table is sorted by the 'id' column.
...
When selecting a new view point, the information set 'data\_sources' and 'data\_sources\_types' attributes are used to select which DBContent is loaded. The 'filters' attribute is used to set the filter active flags and respective conditions. After that, a loading process is triggered. \\
...
\item A Geographic View is reported as a screenshot figure.
\end{itemize}
\end{itemize}
\ \\
...
\item Add Overview Screenshot: Adds a preceding screenshot per Geographic View with a larger map overview and a data marker.
...
The export speed of course depends on the View Points, the number of Views, hardware etc. However, even exporting (somewhat unreasonable) 1500 View Points takes about only 15 minutes on the author's hardware, so it should be adequate for any reasonable use case. \\
