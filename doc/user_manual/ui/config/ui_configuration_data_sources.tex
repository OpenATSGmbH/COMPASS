...
\begin{figure}[H]
    \hspace*{-2cm}
    \includegraphics[width=18cm,frame]{figures/configure_data_sources.png}
...
\item Ground Only: Check if the data source \textbf{only} provides detection for on-ground targets (SMRs)
...
A detection type can be given of any data source, 'Unknown' is given as default. Currently only 'Primary Only' used during reconstruction, but it is very important to set this type for SMRs, to mitigate sub-optimal association of primary-only target reports in close distances to clutter. \\
...
For primary-only data sources, addition JPDA settings for the reconstructor can be given:

\begin{itemize}
\item JPDA PD [1]: Probability of Detection, e.g. 0.9 for 90\%
\item Clutter Rage [1]: Rate of clutter detections per update interval, e.g. 10\\ in the given PSR Maximum range (has to be set below to take effect)
\end{itemize}
\ \\
