...
New Views can be added either to currently existing windows as new tabs, or to a newly opened window. A window can be closed by the close button in the window decoration, which discards all contained Views within the window.  \\
...
Wherever a variable has to be chosen, e.g. for the axes of a Scatter Plot View, the columns of a Table View, a label or a filter condition, 
the 'Select Variable' dialog is opened by clicking the respective variable button. \\

\begin{figure}[H]
  \center
    \fbox{\parbox{0.8\textwidth}{\vspace{1.5cm}\centering \textcolor{red}{\textbf{[FIGURE TODO: 'Select Variable' dialog with the 'Reports' group expanded and an Evaluation report selected]}}\vspace{1.5cm}}}
  \caption{Variable Selection}
\end{figure}

\begin{itemize}
\item 'Search': Filters the shown variables by name, ASTERIX data item or description
\item 'Grouping': The variables are grouped either by ASTERIX 'Data Item' or by variable 'Group' (e.g. 'Position', 'Time', 'Accuracy')
\item Content selection: One radio button per DBContent, and 'Meta' for the Meta Variables. In the \includegraphics[width=0.5cm,frame]{../../data/icons/edit.png} menu, 'Hide Status Categories' hides the DBContents which do not contain target reports (e.g. CAT034)
\item 'Reports': A collapsible second row of radio buttons below the DBContents, one per report offering Report Variables (see \nameref{sec:report_variables})
\item Variable tree: The variables of the selected content, with their data item, data type and description. A variable is selected by clicking 'Select' or by double-clicking it
\item 'Select None': Clears the selection, only shown where an empty selection is allowed
\end{itemize}
\ \\

Please \textbf{note} that some dialogs allow selecting multiple variables at once, e.g. the columns of a Table View. \\
...
Please \textbf{note} that view presets are created for a certain type of view (Geographic View, Histogram View, etc.),
...
\begin{figure}[H]
  ...
  \begin{minipage}{19cm}
    \centering
    \includegraphics[width=9.4cm,frame]{figures/cross_selection_hist.png}\hfill
    \includegraphics[width=9.4cm,frame]{figures/cross_selection_geo.png}
  \end{minipage}
  \caption{Cross-selection between Histogram View and Geographic View}
\end{figure}
...
\begin{figure}[H]
  \hspace*{-0.5cm}
  \centering
  \includegraphics[width=18cm,frame]{figures/view_select_plot.png}
...
All Views color the displayed data by a chosen level of the data-source hierarchy. The Color Mode is set globally and applies consistently to every View. \\\\

The active Color Mode is selected via the Color Mode selector in the 'Data Sources' tool of the Flight Deck (see \nameref{sec:color_mode_selector}). The colors used for each level are configured in the active Data Context (see \nameref{sec:configure_data_context}). \\

\begin{itemize}
\item Available modes
\begin{itemize}
...
\end{itemize}
\item Set globally for the whole application
\begin{itemize}
\item Applies to all open Views consistently
\end{itemize}
\item Each View's Configuration Panel shows the current Color Mode as a read-only label
\item Persisted in the active Data Context
\begin{itemize}
\item Survives restarts and data reloads
\end{itemize}
\end{itemize} \ \\

\begin{figure}[H]
  \centering
    \includegraphics[width=12cm,frame]{figures/ui_color_mode.png}
  \caption{Consistent coloring across Views}
\end{figure}

Please \textbf{note} that the 'Item Mode' panel in the Geographic View (see \nameref{sec:geo_item_mode}) is a separate per-target-report item browser with its own grouping selector, and is not an additional Color Mode level.

\section{Layers Tree}
\label{sec:layers_tree}

The Layers Tree is a shared cross-View widget in the Configuration Panel that exposes the data-source hierarchy and the available view annotations as a single tree. \\\\

It replaces the per-View tree widgets that previously existed (notably in the Geographic View 'Layers' tab) with a unified model. View-specific extras remain documented in the respective View chapter. \\

\begin{itemize}
\item Two top-level groups
\begin{itemize}
\item 'Data': data-source hierarchy, organized as DSType $\rightarrow$ data source $\rightarrow$ line $\rightarrow$ DBContent
\item 'Annotations': all View annotations exposed in a shared place, replacing previously per-View annotation tabs
\end{itemize}
\item Each row carries
\begin{itemize}
\item A visibility checkbox toggling whether the corresponding data is shown
\item The color of its level, when the active Color Mode applies to that level
\end{itemize}
\item Per-group expand/collapse
\begin{itemize}
\item Default expansion depth follows the active Color Mode
\end{itemize}
\end{itemize} \ \\

\begin{figure}[H]
  \centering
    \includegraphics[width=9cm,frame]{figures/ui_layers_tree_dstype.png}
  \caption{Shared Layers Tree: Color per DSType}
\end{figure}

\begin{figure}[H]
  \centering
    \includegraphics[width=9cm,frame]{figures/ui_layers_tree_datasource.png}
  \caption{Shared Layers Tree: Color per Data Source}
\end{figure}