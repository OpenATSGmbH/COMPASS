...
\begin{figure}[H]
    \centering
    \hspace*{-2.5cm}
    \includegraphics[width=19cm,frame]{figures/grid_start.png}
...
The coloring of displayed grid cells due to the visualized statistical values can be customized to the user's needs. \\
...
The Config Tab includes the shared Layers Tree (see \nameref{sec:layers_tree}), which controls per-DBContent / per-Data-Source / per-line visibility for the grid input data. The dedicated per-view annotation tab that existed in earlier releases has been removed; plottable grid annotations of active View Points are now exposed under the 'Annotations' group of the Layers Tree. When multiple grid annotations are present, the 'Plot' and (if available) 'Plot Group' combo boxes are used to choose between them. \\

\begin{figure}[H]
  \centering
    \fbox{\parbox{0.8\textwidth}{\vspace{1.5cm}\centering \textcolor{red}{\textbf{[FIGURE TODO: Grid View Configuration Panel with the shared Layers Tree visible]}}\vspace{1.5cm}}}
  \caption{Grid View: shared Layers Tree in the Config Tab}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Color Scale}: The color scheme used for colorization of grid cells. The following scales are available: 'Green2Red', 'Red2Green', 'Blue2Green2Red', 'Parula', 'Heat', 'Jet', 'Turbo', 'Hot', 'Monochrome Gray', 'Magma', 'Inferno', 'Plasma', 'Viridis', 'Cividis', 'Github', 'Monochrome Red', 'Monochrome Green', 'Monochrome Blue', 'Cubehelix', 'HSV' and 'Custom'.
...
\begin{figure}[H]
  \centering
    \fbox{\parbox{0.8\textwidth}{\vspace{1.5cm}\centering \textcolor{red}{\textbf{[FIGURE TODO: 'Color Scale' combo box expanded showing the full list of available colormaps]}}\vspace{1.5cm}}}
  \caption{Grid View: Color Scale combo box}
\end{figure}

\begin{figure}[H]
  \centering
    \fbox{\parbox{0.8\textwidth}{\vspace{1.5cm}\centering \textcolor{red}{\textbf{[FIGURE TODO: a grid rendered with the new 'Red2Green' colormap]}}\vspace{1.5cm}}}
  \caption{Grid View: Red2Green colormap}
\end{figure}

Please \textbf{note} that the Grid View's Color Scale maps the distributed Z variable and takes priority over the global Color Mode (see \nameref{sec:color_mode}) for the rendered grid cells. The Color Mode still drives the per-layer color used in the Layers Tree and outside of the gridded area. \\
...
Refer to section \nameref{sec:geo_annotation_ops} for how to handle grid items in GeographicView.
