...
\begin{itemize}
 ...
 \item Style: Defines how data is styled. Please refer to section \nameref{sec:style} for details.
...
If the height information is used (3D view) and if target reports without height information are connected, the lines clutter the display. The 'Connect None Height' checkbox allows to set the behavior. \\

Please \textbf{note} that changing both these values requires a manual redraw using the 'Redraw' button.
...
\begin{itemize}
 ...
 \item Rule Generator: Generated e.g. layer-specific rules
 \item Generated Rules: Rules which override the Default Style
\end{itemize}
\  \\
...
\includegraphics[width=0.5cm]{../../data/icons/hint.png} Please also note that for such presets the data from which the style is derived has to be present, otherwise the layer is styled with a common base style.
...
\begin{itemize}
 ...
 \item Color by Num Contributing Sensors: Color is defined by the number of sensors contributing to a CAT062 target report (the number of contributing sensors being defined as the sum of CAT062 REF.CSN.REP and REF.CST.REP items)
...
While the presets come with (mostly) reasonable default values, adaptation can be performed by clicking on the value that should be changed, either in the Default Style or the Generated rules. Any changes are applied immediately to the data.

\subsubsection{Style Examples}

To give a few examples, some interesting Style presets are shown:
