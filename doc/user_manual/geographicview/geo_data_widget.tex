...
\begin{table}[H]
  ...
  \begin{tabular}{ | l | l | l | l |}
    ...
    LMB click & Data & Label & Toggle label \\ \hline
    ...
    RMB click & & Quick menu & Clear labels/selection/measurements/annotations \\ \hline
    RMB click & Data & Data menu & Various actions \\ \hline
    RMB click & Label & Label menu & Copy label cell, toggle, layer actions \\ \hline
...
Please note that if automatic labeling is active, manually created labels are still kept until removed manually, even during reloads.

\subsubsection{Maximized Label}

Existing labels can be maximized to show all (currently available) information about a target report. Simply do a LMB double click on a existing label.
...
The maximized label information is shown until another left click is made.

\subsubsection{Label Menu}

A RMB click on a label opens the label context menu.

\begin{figure}[H]
    \centering
    \includegraphics[width=10cm,frame]{figures/geoview_label_menu.png}
  \caption{Geographic View label context menu}
\end{figure}

The following entries exist:

\begin{itemize}
 \item Copy '\textit{text}': Copies the clicked label cell's text to the clipboard. Only shown when the click hits a non-header cell.
 \item Toggle Label: Hide the label.
 \item Target Report: Submenu with per-report actions. Enabled only for layers that support them:
    \begin{itemize}
     \item Show/Clear ARTAS Target Report Usage: ARTAS tracker layers
     \item Show/Clear MLAT Contributing RUs: CAT020 MLAT layers
    \end{itemize}
 \item Group: Same operations as on the item group's layer menu
 \item Parents: Same operations as on the item's parent layer menu (DBContent, Line, DataSource, DSType)
\end{itemize}

Below the Copy and Target Report entries, the menu duplicates the actions of the Data Menu (see \nameref{sec:data_operations} for details).
...
\begin{figure}[H]
  ...
  \caption{Geographic View data operations}
\end{figure}

The data context menu allows the following operations:

\begin{itemize}
 \item Toggle Label: Toggle label display of a single target report
 \item Group: Same operations as on the item group's layer menu
 \item Parents: Same operations as on the item's parent layer menu
\end{itemize}

Depending on the grouping mode in the 'Config' tab, different parent layer items are shown. For each of these parents the same menu entries exist as if they were clicked in the 'Layers' tab. Please refer to \nameref{sec:data_operations} for details.


\subsubsection{Quick Menu}

Several operations can be performed by a RMB click on the background map (no data).

\begin{figure}[H]
  \center
    \fbox{\parbox{0.8\textwidth}{\vspace{1.5cm}\centering \textcolor{red}{\textbf{[FIGURE TODO: RMB click on the background map, with labels, measurements, a selection, View Point annotations and an imported grid present, so all quick menu entries are shown]}}\vspace{1.5cm}}}
...
\begin{itemize}
 ...
 \item Clear ARTAS Target Report Usage
 \item Clear View Point Annotations: Remove the View Point annotations from the view
 \item Clear Grid Annotations: Remove all imported grids from the view
\end{itemize}

Each entry is only listed if it has something to clear. If nothing can be cleared, no menu is opened. \\
...
When the selection has been done and all target reports in the created latitude/longitude rectangle are selected. This is shown by a different color (yellow by default), and the 'Selected' counter in the lower-left corner shows the current selection size.
...
Please \textbf{note} that the time filter is automatically deactivated if a re-load is triggered.
...
Please note that if the display check is activated for data without height display, bad rendering can occur. For this reason, this mode is only recommended for data display with height and is disabled in 2D display mode.
