...
The root cause is a know bug in one of the used libraries, the Mesa library. The Mesa API acts as the middleman: Applications call OpenGL functions, Mesa's libGL dispatches to the appropriate DRI driver, which Hardware-specific driver executes on GPU. This Mesa libary, in exactly version 23 (and sub-versions) exhibits this issue. \\

Unfortunately, this can not be solved in the application, and no known workarounds exist. The only know solutions are:
\begin{itemize}  
