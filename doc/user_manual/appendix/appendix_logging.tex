...
Logging in COMPASS is performed using the log4cpp library (see \href{https://log4cpp.sourceforge.net/}{Link}). At startup of the application, the logging behavior is defined using a log4cpp.properties file (located in the configuration folder).
...
The log4cpp library always appends to created log files when using FileAppenders.
...
The maxFileSize is the maximum file size in bytes (100 MB in example) after which a new file will be created, of which only maxBackupIndex (5) can exist and are used in rotation.
