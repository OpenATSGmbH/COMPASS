...
Please note that the screenshots are slightly outdated in this chapter.
...
\begin{figure}[H]
 \centering
    \includegraphics[width=15cm]{figures/modes.png}
  \caption{Application Modes}
\end{figure}


The intended use of Live mode is surveillance monitoring: verifying that the surveillance sensors and their network lines deliver data as expected, and the quick inspection of reported issues. Live mode is not intended for operational control, and it is not designed as a permanent recording solution. To make this limitation visible to the users of an installation, the Geographic View can display a configurable status text, for instance 'Not for Control Purposes', as described in \nameref{sec:live_online_offline}. \\
...
In Live mode, most application components are the same as in Offline mode (although some are deactivated), but in the main status bar the Live mode is indicated. 
Using the 'Pause' button the 'Live: Paused' mode can be entered and the 'Stop' button allows stopping the network recording and returning to Offline mode. \\

Activating Live mode will influence the following UI components.

\begin{itemize}
\item Flight Deck - Data Sources: Additional features, see \nameref{sec:data_sources_live_mode}
\item Flight Deck - Sensor Status: Enabled in live mode, see \nameref{sec:sensor_status_live_mode}
\item Geographic View - Special configuration in Live mode, see \nameref{sec:geo_view_live_mode} 
\end{itemize}

\section{Data Sources \& Processing}
\label{sec:data_sources_live_mode}

For each data source, the defined lines are shown (as L1 ... L4 buttons). If data is received over a line, the received data is stored in the database. The line button background color becomes green if data was received on the line during the last 30 seconds, otherwise the button is shown in gray. In 'Live: Paused' mode, a line without any received data is disabled (grayed out). \\
...
\begin{figure}[H]
  \center
    \includegraphics[width=15cm]{figures/live_flow.png}
  \caption{Live Mode Data Flow}
\end{figure}

The two time windows decide where data is inspected from: the most recent 5 minutes are shown in 'Live: Running' using the time window of the Geographic View, older data is loaded from the database in 'Live: Paused' mode. \\

\begin{figure}[H]
  \center
    \includegraphics[width=15cm]{figures/live_data_windows.png}
  \caption{Live Mode Data Windows}
\end{figure}
...
\section{Sensor Status Tool}
\label{sec:sensor_status_live_mode}

\begin{figure}[H]
    \centering
    \includegraphics[width=10cm,frame]{figures/sensor_status_overview.png}
  \caption{Sensor Status Overview}
\end{figure}

When entering Live mode, the Sensor Status tool in the Flight Deck will become available. 
Using this tool, CAT063 sensor status information can be tracked for individual sensors, if available. \\

On top, a system tracker Data Source and Line providing the needed CAT063 data can be selected. \\

Below a list of sensors with their current status exists. The following sensors will be listed:

\begin{itemize}
 \item Sensors which are referenced in the selected tracker line’s CAT063 content, their current status determined by the obtained CON item.
 \item Sensors which are contained in the database but not in the selected tracker line’s CAT063 content, their current status set to \textcolor{gray}{Unknown}.
\end{itemize}
\  \\

CON statuses include:

\begin{itemize}
 \item \textcolor{green!50!black}{Operational}
 \item \textcolor{orange}{Degraded}
 \item \textcolor{blue}{Initializing}
 \item \textcolor{red}{Not Connected}
\end{itemize}
\  \\

Additionally, if the last received sensor update is older than a defined period, the sensor status will be set to \textcolor{gray}{Unknown} until a new sensor update is received for the respective sensor.

This period can be specified by opening the tool's configuration menu \includegraphics[width=0.5cm,frame]{../../data/icons/edit.png} and selecting one of the predefined values in the sub-menu \textit{Maximum Status Age}. \\

For each listed sensor, the time of day of the sensor's last received update can be displayed by selecting \textit{Show Last Update ToD} in the configuration menu. \\

\begin{figure}[H]
    \centering
    \includegraphics[width=12cm,frame]{figures/sensor_status_detail.png}
  \caption{Sensor Status: Show Last Update ToD}
\end{figure}

Below the sensor list, two items are displayed:

\begin{itemize}
 \item Received Time of Day: The received last sensor update’s time of day
 \item Updated Time of Day: The last time of day the display has been updated
\end{itemize}
\  \\

At the bottom, a text box lists all important updates and status changes of the selected tracker line.

\begin{figure}[H]
    \centering
    \includegraphics[width=14cm,frame]{figures/sensor_status_log.png}
  \caption{Sensor Status: Sensor Status Log}
\end{figure}

Status changes are displayed as:

Time of Day | Sensor Name | Old State $\rightarrow$ New State

\section{Geographic View}
\label{sec:geo_view_live_mode}
...
The text captions are configurable, so an installation can display an intended-use note instead, for instance 'Not for Control Purposes' as the offline state text or as both texts.
...
Per default, the RAM cache is limited to 5 minutes, while the database content is limited to 60 minutes. To change these durations (if workstation performance allows for it), the following values can be set in the file 'compass.json' (values in minutes):
